...
public class Search {
	...
	public static void modifyContent(List<SearchDTO> list) { // content 내용을 수정하는 메서드
		...
		for (int b = 0; b < list.size(); b++) {
			...
			list.get(b).setContent(content); // dto에 쓸데없는 부분 제거한 content를 다시 대입, SearchBean에서 modifyContent 메소드 실행으로
												// contentUpdate 가능!
		}
	}
	
	public static String modifyBoardName(String board) {
		...
		case "living":
			...
			break;
		case "market":
			boardName = "marketBoard";
			break;
		case "diy":
			boardName = "diyBoard";
			break;
		case "community":
			boardName = "communityBoard";
			break;
		}
		return boardName;
	}
	...
	public static String modifySearchBoardName(String board) {
		...
		switch(board) {
		case "living":
			searchBoardName = "생활정보 게시판";
			break;
		case "market":
			searchBoardName = "장터 게시판";
			break;
		case "diy":
			searchBoardName = "집꾸미기 게시판";
			break;
		case "community":
			searchBoardName = "자유 게시판";
			break;
		}
		return searchBoardName;
	}
