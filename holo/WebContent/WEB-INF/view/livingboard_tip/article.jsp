...
<script>
	...
	function getReplyList(){
		...
		$.ajax({
			type:'POST',
			url: "/holo/livingboard_tip/replyList.holo",
			data: paramData,
			dataType: 'JSON',
			success: function(result){
				...
				if(result.length < 1){
					...
					$(result).each(function(){
	                    ...
	                    htmls += '<button id="replikesUpdate" onclick="replikesUpdate_click('+this.repnum+')" style="padding:1px 1px; font-size:0.5px;">';
	        			htmls += '👍 ';
	        			htmls += '<span id="replikesCount">'+this.likes+'</span></button> &nbsp;';
	        			htmls += '<button onclick="reportReply('+this.repnum+')" style="padding:2px 4px; font-size:1px;">📢</button></td>'
    ...
    function reportArticle(articlenum, subject){
    	if(!${sessionCheck}){
			...
			window.open("/holo/livingboard_tip/reportArticle.holo?articlenum="+articlenum+"&subject="+subject, "a", "width=700, height=700, left=100, top=50");
		}
	}
	    	...
	    	$.ajax({
	    		...
	    		success: function(result){
	    			if(result==1){
	    				...
	    				window.open("/holo/livingboard_tip/reportReply.holo?repnum="+repnum, "a", "width=500, height=300, left=100, top=50");
	    			}
	    		}
	    	})
		}
...
<h7><a href="/holo/livingboard/list.holo">생활Tip</a>-<a href="/holo/livingboard_tip/list.holo">tip게시판</a></h7>
	...
	<tr>
		<td colspan="4" align="right">
			<c:if test="${sessionScope.sessionId == dto.id}">
				<input type="button" value="수정하기" onclick="window.location='/holo/livingboard_tip/updateForm.holo?articlenum=${dto.articlenum}&pageNum=${pageNum}'" />
				<input type="button" value="글삭제" onclick="delConfirm('/holo/livingboard_tip/deleteArticle.holo?articlenum=${dto.articlenum}&pageNum=${pageNum}')" />
			</c:if>
			<input type="button" value="목록으로" onclick="window.location='/holo/livingboard_tip/list.holo?pageNum=${pageNum}'" />
		</td>
	</tr>
	<tr>
...
<form method="POST" action="/holo/livingboard_tip/insertRep.holo">
	<tr>
