...
<table border="1" style="border-collapse:collapse">
	...
	<tr>
		...
		<div align="left">
			...
			- <a class="category" href="/holo/livingboard_tip/list.holo">Tip게시판</a>
			<hr >
...
<c:if test="${count>0}">
	<c:forEach var="list" items="${articleList}">
		<tr>
			...
			<td><a class="title" href="/holo/livingboard_tip/article.holo?articlenum=${list.articlenum}&pageNum=${currentPage}">${list.subject} 
...
<c:if test="${count>0}">
	<c:if test="${startPage>5}">
		<a class="pages" href="/holo/livingboard_tip/list.holo?pageNum=${startPage-1}">[이전]</a>
	</c:if>
	<c:forEach var="pagenum" begin="${startPage}" end="${endPage}">
		<a class="pages" href="/holo/livingboard_tip/list.holo?pageNum=${pagenum}">${pagenum}</a>
	</c:forEach>
	<c:if test="${endPage<pageCount}">
		<a class="pages" href="/holo/livingboard_tip/list.holo?pageNum=${startPage+5}">[다음]</a>
	</c:if>
</c:if>

<form action="/holo/livingboard_tip/searchList.holo" method="post">
<select name="sort">
...
<input type="hidden" name="category_b" value="tip" />
