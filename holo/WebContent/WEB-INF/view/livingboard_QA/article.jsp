...
<script>
	...
	function getReplyList(){
		...
		$.ajax({
			type:'POST',
			url: "/holo/livingboard_QA/replyList.holo",
			data: paramData,
			dataType: 'JSON',
	...
	function reportArticle(articlenum, subject){
    	if(!${sessionCheck}){
			...
			window.open("/holo/livingboard_QA/reportArticle.holo?articlenum="+articlenum+"&subject="+subject, "a", "width=700, height=700, left=100, top=50");
		}
	}
	    	...
	    	$.ajax({
	    		...
	    		success: function(result){
	    			if(result==1){
	    				...
	    				window.open("/holo/livingboard_QA/reportReply.holo?repnum="+repnum, "a", "width=500, height=300, left=100, top=50");
	    			}
	    		}
	    	})
		}
...
<h7><a href="/holo/livingboard/list.holo">생활Tip</a><a href="/holo/livingboard_QA/list.holo">- 질문게시판</a></h7>
	...
	<tr>
		<td colspan="4" align="right">
			<c:if test="${sessionScope.sessionId == dto.id}">
				<input type="button" value="수정하기" onclick="window.location='/holo/livingboard_QA/updateForm.holo?articlenum=${dto.articlenum}&pageNum=${pageNum}'" />
				<input type="button" value="글삭제" onclick="delConfirm('/holo/livingboard_QA/deleteArticle.holo?articlenum=${dto.articlenum}&pageNum=${pageNum}')" />
			</c:if>
			<input type="button" value="목록으로" onclick="window.location='/holo/livingboard_QA/list.holo?pageNum=${pageNum}'" />
		</td>
	</tr>
	<tr>
...
<c:if test="${sessionScope.sessionId != null }">
	<form method="POST" action="/holo/livingboard_QA/insertRep.holo">
		<tr>
